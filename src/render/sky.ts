...
export class Sky {
  ...
  constructor(moonTex: THREE.Texture) {
    ...
    // each star twinkles on its own clock — phase and rate per star
    const n = 90;
    ...
    for (let i = 0; i < n; i++) {
      ...
      rate[i] = 0.4 + Math.random() * 1.8;      // slow breathers and quick flickerers
      size[i] = i % 9 === 0 ? 2 : 1;            // the occasional brighter star
    }
    ...
    this.starMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        attribute float aPhase, aRate, aSize;
        varying float vTwinkle;
        uniform float uTime;
        void main() {
          vTwinkle = 0.35 + 0.65 * (0.5 + 0.5 * sin(uTime * aRate + aPhase));
          gl_PointSize = aSize;
      ...
      fragmentShader: `
        ...
        void main() { gl_FragColor = vec4(0.74, 0.72, 0.86, uAlpha * vTwinkle); }`,
      transparent: true,
      depthWrite: false,
    });
